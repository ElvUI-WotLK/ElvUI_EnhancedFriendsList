...
local textFormat = 
locale == "ruRU" and "|TInterface\\Glues\\CharacterCreate\\UI-CharacterCreate-Classes:20:20:0:0:64:64:%d:%d:%d:%d|t %s%s|r, %s%d-го уровня" or 
locale == "deDE" and "|TInterface\\Glues\\CharacterCreate\\UI-CharacterCreate-Classes:20:20:0:0:64:64:%d:%d:%d:%d|t %s%s|r, %sStufe %d" or 
"|TInterface\\Glues\\CharacterCreate\\UI-CharacterCreate-Classes:20:20:0:0:64:64:%d:%d:%d:%d|t %s%s|r, %sLevel %d";
		...
		if(friend.id and friend.buttonType == FRIENDS_BUTTON_TYPE_WOW) then
			...
			if(connected) then
				if(locclasses[class]) then
					...
					friend.name:SetFormattedText(textFormat, (left + 0.015)*64, (right - 0.02)*64, (top + 0.018)*64, (bottom - 0.02)*64, E:RGBToHex(color.r, color.g, color.b), name, E:RGBToHex(diffColor.r, diffColor.g, diffColor.b), level);
				...
				if(ElvCharacterDB.FriendsListColor[name]) then
					...
					diffColor = GetQuestDifficultyColor(level);
					friend.name:SetFormattedText(textFormat, (left + 0.015)*64, (right - 0.02)*64, (top + 0.018)*64, (bottom - 0.02)*64, E:RGBToHex(color.r*0.5, color.g*0.5, color.b*0.5), name, E:RGBToHex(diffColor.r*0.5, diffColor.g*0.5, diffColor.b*0.5), level);
